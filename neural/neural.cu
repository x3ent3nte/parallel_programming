...
void initNeurons(Neuron* layer, int layer_size) {
    ...
    layer[global_index] = Neuron{1.0f, 0.0f, 0.0f, 0.0f};
}
...
void computeDeltas(Neuron* layer,
    ...
    for (int i = 0; i < next_layer_size; i++) {
        ...
        sum_delta_weights += (i * layer_size + global_index) * infront_delta;
    }
    layer[global_index].delta = layer[global_index].sum * sigmoidPrime(sum_delta_weights);
}
...
int main() {
    ...
    Neuron* d_hidden_layer;
    cudaMalloc(&d_hidden_layer, sizeof(Neuron) * hidden_size);

    Neuron* d_output_layer;
    cudaMalloc(&d_output_layer, sizeof(Neuron) * output_size);

    float* d_hidden_weights;
    cudaMalloc(&d_hidden_weights, sizeof(float) * hidden_size * input_size);

    float* d_output_weights;
    cudaMalloc(&d_output_weights, sizeof(float) * output_size * hidden_size);

    float* d_hidden_sig_x_weights;
    cudaMalloc(&d_hidden_sig_x_weights, sizeof(float) * hidden_size * input_size);

    float* d_output_sig_x_weights;
    cudaMalloc(&d_output_sig_x_weights, sizeof(float) * output_size * hidden_size);

    initNeurons<<<1, input_size>>>(d_input_layer, input_size);
    initNeurons<<<1, hidden_size>>>(d_hidden_layer, hidden_size);
    initNeurons<<<1, output_size>>>(d_output_layer, output_size);

    fillRandomFloatBetween01<<<1, hidden_size * input_size>>>(d_hidden_weights, hidden_size * input_size, seed);
    fillRandomFloatBetween01<<<1, output_size * hidden_size>>>(d_output_weights, output_size * hidden_size, seed);
    ...
    for (int i = 0 ; i < 100; i++) {
        ...
        for (int j = 0; j < num_trials; j++) {
            ...
            computeLayer<<<1, hidden_size>>>(d_hidden_layer, d_input_layer, hidden_size, input_size, d_hidden_weights, d_hidden_sig_x_weights);
            computeLayer<<<1, output_size>>>(d_output_layer, d_hidden_layer, output_size, hidden_size, d_output_weights, d_output_sig_x_weights);
            ...
            computeDeltas<<<1, hidden_size>>>(d_hidden_layer, d_output_layer, d_output_weights, hidden_size, output_size);

            adjustWeights<<<1, output_size>>>(d_output_layer, output_size, hidden_size, d_output_weights, d_output_sig_x_weights);
            adjustWeights<<<1, hidden_size>>>(d_hidden_layer, hidden_size, input_size, d_hidden_weights, d_hidden_sig_x_weights);
